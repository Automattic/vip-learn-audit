...
class Command {
	...
	public function course_title_case_audit( $args ) {
		...
		WP_CLI\Utils\format_items( 'table', $rows, array( 'Current Title', 'Title Case', 'Edit Link' ) );
	}
	...
	public function course_sentence_case_audit( $args ) {
		...
		WP_CLI\Utils\format_items( 'table', $rows, array( 'Current Title', 'Sentence Case', 'Edit Link' ) );
	}
	...
	public function word_instance_audit( $args ) {
		...
		// Display the results in a table.
		WP_CLI\Utils\format_items( 'table', $rows, array( 'Page Title', 'Word Instance', 'Correct Word Instance', 'Edit Link' ) );
	}
	...
	public function course_punctuation_audit( $args ) {
		...
		foreach ( $lessons as $lesson ) {
			...
			foreach ( array( 'p', 'li', 'blockquote' ) as $tag ) {
		...
		// Display the results in a table.
		WP_CLI\Utils\format_items( 'table', $rows, array( 'Text', 'LastChar', 'Edit Link' ) );
	}
	...
	public function link_audit( $args ) {
		...
		// Display the results in a table.
		WP_CLI\Utils\format_items( 'table', $rows, array( 'Lesson URL', 'Link Text', 'Link URL', 'Edit Link' ) );
	}
	...
	public function completion_times_audit( $args ) {
		...
		$rows[] = array(
			...
			'Median time'    => $total_median_time . ' (' . gmdate( 'H:i', $total_median_time ) . ' hours)',
		...
		if ( empty( $rows ) ) {
			...
			// Display the results in a table.
			WP_CLI\Utils\format_items( 'table', $rows, array( 'Name', 'Reading time', 'Median time', 'Estimated time' ) );
		}
	}
...
if ( class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'vip-learn audit course-title-case', array( new Command(), 'course_title_case_audit' ) );
	WP_CLI::add_command( 'vip-learn audit course-sentence-case', array( new Command(), 'course_sentence_case_audit' ) );
	WP_CLI::add_command( 'vip-learn audit course-punctuation', array( new Command(), 'course_punctuation_audit' ) );
	WP_CLI::add_command( 'vip-learn audit check-title-case-string', array( new Command(), 'check_title_case' ) );
	WP_CLI::add_command( 'vip-learn audit check-sentence-case-string', array( new Command(), 'check_sentence_case' ) );
	WP_CLI::add_command( 'vip-learn audit word-instances', array( new Command(), 'word_instance_audit' ) );
	WP_CLI::add_command( 'vip-learn audit links', array( new Command(), 'link_audit' ) );
	WP_CLI::add_command( 'vip-learn audit completion-times', array( new Command(), 'completion_times_audit' ) );
}
